...
These egress FQDNs must be opened at the hub Azure Firewall for force-tunneled spokes; see `alz-firewall-ops/FIREWALL-EGRESS-IMPLEMENTED.md` (canonical). Sub-5 personal runners use NAT Gateway egress and are exempt from the hub firewall, but the same dependencies are documented for audit and future migration.
